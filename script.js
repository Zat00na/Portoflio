/* ============================================================
   ...
   Portfolio Script (Data-Driven with Built-in Fallback)
   ============================================================ */
  ...
  skills: [
    {
      ...
      items: [
        ...
        { name: "2D Character Animation", icon: "play" },
        { name: "Toon Boom Harmony", icon: "brush" },
        { name: "Kinetic Typography", icon: "type" },
        { name: "Lottie / Web Animation", icon: "code" }
      ]
    },
    {
      group: "3D Art & VFX",
      icon: "cube",
      items: [
        { name: "Blender 3D", icon: "cube" },
        { name: "Cinema 4D", icon: "cube" },
        { name: "Lighting & Shading", icon: "sun" },
        { name: "Octane & Cycles", icon: "render" },
        { name: "3D Camera Tracking", icon: "camera" }
      ]
    },
    {
      ...
      items: [
        ...
        { name: "Visual Identity Systems", icon: "grid" },
        { name: "Poster & Editorial Design", icon: "layout" },
    ...
    },
    {
      ...
      icon: "sparkles",
      items: [
        { name: "Storyboarding & Animatic", icon: "film" },
        { name: "Concept & Color Scripting", icon: "palette" },
        { name: "Visual Development", icon: "brush" },
        { name: "Sound Design & Sync", icon: "music" },
        { name: "Styleframes & Pitching", icon: "grid" }
      ]
    }
  ],
  projects: [
    {
      ...
      poster: "assets/project1-thumb.svg",
    ...
    },
    {
      ...
      poster: "assets/project2-thumb.svg",
    ...
    },
    {
      ...
      videoUrl: "https://assets.mixkit.co/videos/preview/mixkit-geometric-animation-with-neon-shapes-and-lines-41584-large.mp4",
      poster: "assets/project3-thumb.svg",
    ...
    },
    {
      ...
      images: [
        { src: "assets/project4-img1.svg", alt: "Aura Botanic Brand Identity" },
        { src: "assets/project4-img2.svg", alt: "Aura Botanic Packaging Details" }
      ],
...
function videoHtml(project) {
  return `
    <div class="project-video-wrap" data-video-id="${esc(project.id)}">
      <video class="project-video" poster="${esc(project.poster || '')}" preload="metadata" playsinline loop controls>
...
function renderHero(data) {
  ...
  if (heroImg) {
    heroImg.src = hero.profileImage || 'assets/profile.svg';
...
/* ─── RENDER: PROJECTS (EXPANDED MEDIA + VIDEO SUPPORT) ───── */
function renderProjects(data) {
  const cards = data.projects.map(p => {
    let mediaHtml = '';
    if (p.type === 'video' && p.videoUrl) {
      mediaHtml = videoHtml(p);
    } else if (p.images && p.images.length) {
      mediaHtml = galleryImagesHtml(p.images);
    } else {
      mediaHtml = `<img src="${esc(p.poster || '')}" alt="${esc(p.title)}" class="project-img active" />`;
    }
...
function initGalleries() {
  document.querySelectorAll('.project-card').forEach(card => {
    ...
    function start() {
      timer = setInterval(() => show((current + 1) % imgs.length), 3800);
    }
    function stop() {
      clearInterval(timer);
    }
...
function initVideoPlayers() {
  document.querySelectorAll('.project-video-wrap').forEach(wrap => {
    ...
    const btn = wrap.querySelector('.project-video-btn');
    if (!video || !btn) return;
